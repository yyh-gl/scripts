...
usage() {
	echo "\nSafari 検索"
	echo "Usage: safari <option> [argument1]"
	echo 
	echo "オプション:"
...
if [[ -n "${opthash[(i)-h]}" ]]; then
	...
	open -a Safari https://mail.google.com/mail/ca/u/0/\#inbox
elif [[ -n "${opthash[(i)-c]}" ]]; then
	open -a Safari https://calendar.google.com/calendar/render\?tab=Tc\#main_7
else
	if [ $# != 0 ]; then
		for i in $*; do
